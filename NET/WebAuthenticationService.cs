public class WebAuthenticationService : IAuthenticationService<int>
{
    ...
    public async Task LogInAsync(IUserAuthData user, params Claim[] extraClaims)
    {
        ...
        identity.AddTenantId(user.OrganizationId);

        if (user.OrgRoles != null && user.OrgRoles.Any())
        {
            foreach (string singleOrgRole in user.OrgRoles)
            {
                identity.AddClaim(new Claim(ClaimsIdentity.DefaultRoleClaimType, singleOrgRole, ClaimValueTypes.String));
            }
        }
        ...
        foreach (var claim in identity.Claims)
        {
            switch (claim.Type)
            {
                ...
                default:
                    ...
                    {
                        baseUser.OrganizationId = claim.Value;
                    }
